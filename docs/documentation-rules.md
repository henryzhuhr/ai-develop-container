...
- `docs/issues/`
  - Issue 文档，只回答“现在有什么问题、影响是什么、准备怎么修、当前状态是什么”。
- `docs/guides/`
  - 使用文档，只回答“如何使用、如何操作”。
- `docs/ops/`
...
- 需求专题文件统一放在 `docs/requirements/` 下，例如：
  ...
  - `vscode-remote.md`
  - `verification.md`
- Issue 专题文件统一放在 `docs/issues/` 下，例如：
  - `ssh-git-mount-compatibility.md`
...
`docs/issues/` 下的每个 issue 文档，默认按以下结构编写：
...
- 所有可执行、可跟踪的需求都必须带编号。
- 主需求文档使用 `R-001`、`R-002` 这类全局编号。
- 主需求下的细分要求可使用 `R-002.1`、`R-002.2` 这类子编号。
- 子需求文档使用独立前缀编号：
  - Dev Container: `DC-001`
  - VS Code Remote: `VR-001`
  - Verification: `VF-001`
- Issue 文档使用独立前缀编号：
  - Issue: `ISS-001`
- 后续新增专题时，也必须定义稳定前缀，避免无编号需求。
- 与用户沟通时，优先直接引用需求编号。

## 6. 内容边界

- 需求文档不写具体运行命令，命令放到 `guides/`。
- Issue 文档可以保留最小必要的复现命令、错误输出和目标修复方向，但不应替代最终使用说明。
...
- 新增需求前，先判断是否属于已有专题。
- 新增 issue 前，先判断是否属于已知缺陷、兼容性问题或待修复项；如果是，优先放到 `docs/issues/`。
